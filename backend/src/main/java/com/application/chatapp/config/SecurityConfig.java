    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, OncePerRequestFilter jwtAuthFilter)
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // Allow WebSocket handshake for authenticated users
                        .requestMatchers("/ws-chat/**").authenticated()
                        // All chat, attachment, and other endpoints require valid JWT
                        .requestMatchers("/chat/**", "/attachment/**", "/chatroom/**").authenticated()
                        .anyRequest().authenticated())
                .sessionManagement(sm -> sm.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .addFilterBefore(jwtAuthFilter, UsernamePasswordAuthenticationFilter.class);
